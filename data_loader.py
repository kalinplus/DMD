...
class MMDataset_modality_level(Dataset):
    ...
    def __init__(self, args, contribution_t, contribution_v, contribution_a, alpha, func='linear', mode='train'):
        ...
        # 根据数据集调用对应函数，参数来自 train.py
        DATASET_MAP[args['dataset_name']](contribution_t, contribution_v, contribution_a, alpha, func)
    
    # __init_mosi 会被调用 3 次，分别加载训练、验证、测试集
    def __init_mosi(self, contribution_t, contribution_v, contribution_a, alpha, func='linear'):
        # 加载并初始化默认的 数据文本、视觉和语音模态的特征
        '''
        特征维度应该是：
        第 0 维：样本数量
        ...
        if func == 'linear':
            ...
            tanh = torch.nn.Tanh()
            difference = tanh(torch.tensor((difference_base / 3 * 2 ) * alpha))
        elif func == 'square':
            difference = (difference_base / 3 * 2 ) ** 1.5 * alpha
